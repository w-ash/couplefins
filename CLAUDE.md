...
1. **Settlement** — "Who owes whom?" Each transaction has a `payer_percentage` (0-100). When < 100, the difference is owed to the payer. Every settlement explicitly records the months it covers as (year, month, amount) portions summing to the settlement amount (v1.11.0) — one portion for the monthly rent transfer, several for a catch-up lump (allocated at record time, oldest covered month first, remainder on the last). A month's balance is its charges' shares net minus the portions allocated to it; a year is the sum of its months, and Settle Up shows one year at a time with no all-time figure. Coverage is never inferred and portions are never reallocated — display math is purely additive, computed in Python and served precomputed.
