...
async def test_previews_all_transactions() -> None:
    uow = make_mock_uow()
    person = make_person()
    ...
    assert result.transactions[0].merchant == "Grocery Store"
    assert result.transactions[0].is_shared is True
    assert result.transactions[0].payer_percentage == 50
    assert result.transactions[2].payer_percentage == 70
